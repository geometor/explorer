...
def _create_section_from_points(points: list[spg.Point], model: Model) -> dict:
    ...
    return {
        ...
        "lengths": [float(length) for length in lengths_val],
        "decimal_lengths": [f"{length:.4f}" for length in lengths_val],
        "latex_lengths": [sp.latex(length) for length in section.lengths],
...
def to_browser_dict(model: Model) -> dict:
    ...
    for el, data in model.items():
        ...
        element_dict = {
            ...
            "parents": [
                model[p].ID for p in data.parents.keys() if p in model and model[p].ID
            ],
        ...
        if isinstance(el, spg.Point):
            ...
            element_dict.update(
                {
                    ...
                    "latex_coefficients": [
                        sp.latex(clean_expr(c)) for c in el.coefficients
                    ],
            ...
            element_dict.update(
                {
                    ...
                    "degree_angles": {
                        model[p].ID: f"{a * 180 / sp.pi:.3f}°"
                        for p, a in angles_val.items()
                    },
                    "latex_angles": {
                        model[p].ID: sp.latex(clean_expr(a))
                        for p, a in el.angles.items()
                    },
                    "spreads": spreads,
                    "area": float(area_val),
            ...
            isinstance(el, sp.Expr) and data.object == el
        ):  # Check if it's a polynomial expression from a Polynomial element
