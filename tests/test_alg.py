...
def has_nested_radicals(expr):
    ...
    radicals = expr.atoms(sp.sqrt) | {
        p for p in expr.atoms(sp.Pow) if p.exp == sp.Rational(1, 2)
    }
    ...
    for rad in radicals:
        ...
        base_radicals = base.atoms(sp.sqrt) | {
            p for p in base.atoms(sp.Pow) if p.exp == sp.Rational(1, 2)
        }
